...
export class FixAfterMergeCisComponent {
    ...
    of(0)
      .pipe(
        ...
        switchMap(() => {
          ...
          const cisTarget =
            this.data.outlet['mid_target']?.cis ?? this.cisAfter;
          return this.comparingCis(
  ...
  private getUser() {
    ...
    const listCis = this.listCisBefore.filter(
      (cisBefore) => cisBefore.trim() != '',
    );
    ...
    return this.getAuthMessi().pipe(
      switchMap((response: any) => {
        ...
        if (!queryValue) {
          throw new Error(
            'Email/CIS query is empty. Cannot fetch user from Messi.',
          );
        }
  ...
  private filterListMid() {
    ...
    return from(listNotFilteredQrisStatis).pipe(
      concatMap((mid) =>
        this.httpClient
          .get(MAGENTA_GET_STICKER_QRIS_BY_MID.replace('{mid}', mid), options)
          .pipe(
            tap((response: any) => {
              if (response.status != 200) {
            ...
            catchError((error) => {
              if (error.status == 409) {
                this.addLogs({
                  date: new Date(),
                  message: `Sticker For MID ${mid} is not Found`,
                  level: 'MESSAGE',
                  status: 'FAILED',
                });

import { Component, Input, ChangeDetectionStrategy, DestroyRef, inject } from '@angular/core';
...
export class QuickHandleComponent {
  ...
  submitQuickHandleManual(){
    ...
    const body : any = JSON.parse(JSON.stringify({
    ...
    this.httpClient.post(url, body, option).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (response : any) => {
  ...
  submitQuickHandleSetUnhandled(){
    ...
    const option : any = {observe : "response"};
    this.statusQuickHandle = StatusAPI.LOADING;
    this.httpClient.delete(url, option).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (response : any) => {
        if(response.status != 200)
        {
          this.statusQuickHandle = StatusAPI.FAILED;

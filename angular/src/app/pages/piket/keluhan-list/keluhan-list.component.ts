...
export class KeluhanListComponent implements OnInit {
  ...
  private isBatchDueSoon(keluhan: KeluhanList): boolean {
    return this.toDayKey(new Date(keluhan.duedate)) == this.toDayKey(new Date()) && keluhan.status.toLowerCase() == 'assigned';
  }
  ...
  getData(): void {
    ...
    this.httpClient.get(url, option).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (response: any) => {
        if (response.status != 200) {
          this.statusMenu.set(StatusAPI.FAILED);
          this.statusList.set(StatusAPI.FAILED);
          this.errorMessage.set(`Gagal memuat list keluhan (HTTP ${response.status}).`);
          return;
        }
        const newRows: KeluhanListRow[] = (response.body.content as KeluhanList[]).map((keluhan) => ({
          ...keluhan,
        ...
        const listRequestId = newRows.map((keluhan: KeluhanList) => keluhan.requestid);
      ...
      error: (err: any) => {
        this.statusMenu.set(StatusAPI.FAILED);
        this.statusList.set(StatusAPI.FAILED);
        this.errorMessage.set(`Gagal memuat list keluhan. ${err?.message ?? ''}`.trim());
      }
    })
  }
  ...
  getHandledRequestId(requestId: string[]) {
    ...
    this.httpClient.get(url, option).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      ...
      error: (err: any) => {
        ...
        this.errorMessage.set(`Gagal memuat informasi penanganan. ${err?.message ?? ''}`.trim());
      }
    })
  }

...
import {
  Encoding, SslType, OutputFormat, DetectionResult, SslMetadata, ConvertOutput, KeyAlg,
  detect, parseMetadata, availableOutputs, resolveKeyAlg, convert, OUTPUT_LABELS,
} from './ssl-converter.logic';

interface OutButton { fmt: OutputFormat; label: string; hint: string; }
...
export class SslConverterComponent {
  ...
  readonly supported = ['.pem', '.cer', '.crt', '.der', '.key', '.p7b', '.p7c', '.csr', '.pub', '.p12', '.pfx', '.jks'];
  ...
  get hasInput(): boolean { return !!this.detection || this.rawText.trim().length > 0 || this.byteCount > 0; }
  ...
  get previewTruncated(): boolean { return (this.output?.text?.length ?? 0) > this.previewCap; }
  ...
  iconForType(t: SslType | undefined): string {
    switch (t) {
      case SslType.CERT: return 'verified';
      case SslType.CSR: return 'assignment';
      case SslType.PRV_PKCS1:
      case SslType.PRV_PKCS8:
      case SslType.PRV_PKCS5: return 'vpn_key';
      case SslType.PUB_KEY: return 'key';
      case SslType.P7B: return 'layers';
      case SslType.CRL: return 'block';
      case SslType.PKCS12: return 'inventory_2';
      case SslType.JKS: return 'storage';
  ...
  onInput(): void {
    this.error = '';
    if (!this.rawText.trim()) { this.resetResult(); return; }
    this.runDetect(this.rawText);
  }

  onDragOver(ev: DragEvent): void { ev.preventDefault(); this.dragActive = true; }
  onDragLeave(ev: DragEvent): void {
    ...
    if (!zone || !zone.contains(ev.relatedTarget as Node | null)) this.dragActive = false;
  }
  ...
  private runDetect(input: string | Uint8Array, fileName?: string): void {
    ...
    if (d.type === SslType.UNKNOWN) {
      ...
      if (d.encoding !== Encoding.UNKNOWN || input instanceof Uint8Array || (typeof input === 'string' && input.trim().length > 0)) {
        this.error = 'Format tidak dikenali. Tempel PEM yang valid atau unggah file SSL.';
      }
      return;
    }
  ...
  private toButtons(fmts: OutputFormat[]): OutButton[] {
    return fmts.map((fmt) => ({ fmt, label: OUTPUT_LABELS[fmt], hint: this.hints[fmt] }));
  }
  ...
  private tryDecrypt(): void {
    ...
    this.outputs = this.toButtons(availableOutputs(this.detection, this.keyAlg));
  ...
  copy(): void {
    ...
    if (!navigator.clipboard?.writeText) {
      this.error = 'Clipboard tidak tersedia di konteks ini (gunakan HTTPS atau localhost).';
      return;
    }

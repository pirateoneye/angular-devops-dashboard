import { Component, ContentChildren, QueryList, AfterContentInit, HostListener, ElementRef } from '@angular/core';
...
export class MsvMenuComponent implements AfterContentInit {
  @ContentChildren(MsvMenuItemComponent) menuItems!: QueryList<MsvMenuItemComponent>;
  ...
  private focusPreviousItem(enabledItems: MsvMenuItemComponent[]): void {
    const currentIndex = this.getCurrentEnabledIndex(enabledItems);
    const previousIndex = currentIndex <= 0 ? enabledItems.length - 1 : currentIndex - 1;
  ...
  focusFirstEnabledItem(): void {
    const enabledItems = this.menuItems.toArray().filter(item => !item.disabled);

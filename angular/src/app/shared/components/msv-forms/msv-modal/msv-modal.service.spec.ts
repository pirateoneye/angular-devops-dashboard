...
describe('MsvModalService', () => {
  ...
  it('should open modal with component', () => {
    ...
    expect(modalRef.componentInstance).toBeInstanceOf(TestModalContentComponent);
  ...
  it('should set closable to false when configured', () => {
    const modalRef = service.open(TestModalContentComponent, { closable: false });
    expect(modalRef).toBeTruthy();
    modalRef.close();
  });

  it('should set closable to true when configured', () => {
    const modalRef = service.open(TestModalContentComponent, { closable: true });
    expect(modalRef).toBeTruthy();
    modalRef.close();
  });
  ...
  it('should emit result when modal is closed with value', (done) => {
    const modalRef = service.open<TestModalContentComponent, string>(TestModalContentComponent);
  ...
  it('should open confirmation dialog', () => {
    ...
    // Clean up
    const overlayContainer = document.querySelector('.cdk-overlay-container');
    const cancelBtn = overlayContainer?.querySelector('.msv-confirm-cancel') as HTMLButtonElement;
    cancelBtn?.click();
  });
  ...
  it('should return true when confirm dialog is accepted', (done) => {
    ...
    setTimeout(() => {
      const overlayContainer = document.querySelector('.cdk-overlay-container');
      const okBtn = overlayContainer?.querySelector('.msv-confirm-ok') as HTMLButtonElement;
      okBtn?.click();
    }, 50);
  });
  ...
  it('should clean up component views when modal closes', (done) => {
    ...
    modalRef.afterClosed$.subscribe(() => {
      ...
      setTimeout(() => {
        const overlayContainer = document.querySelector('.cdk-overlay-container');
        const testComponent = overlayContainer?.querySelector('.test-component');

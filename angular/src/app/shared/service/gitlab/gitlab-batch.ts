...
type MrMode = 'single' | 'chained';
type BranchMode = 'same' | 'per-project';

export const DEFAULT_CONCURRENCY = 3;

interface SingleMrOptions {
  mode: 'single';
...
type CreateMrsOptions = SingleMrOptions | ChainedMrOptions;
...
export async function createMergeRequests(
  ...
  const perProject = mapWithConcurrency(
    projectIds,
    limit,
    async (pid): Promise<MrResult[]> => {
      if (opts.mode === 'single') {
...
interface CreateTagBatchOptions {
  tag_name: string;
  ref: string;
  message?: string;
}
export function createTags(
  ...
  return runSimple(
    ...
    nameOf,
    'tag',
    (pid) => client.createTag(pid, opts),
    () => `${opts.tag_name} @ ${opts.ref}`,
    batch,
  );
}

interface CreateBranchBatchOptions {
  branch: string;
  ref: string;
}
export function createBranches(
  ...
  return runSimple(
    ...
    nameOf,
    'branch',
    (pid) => client.createBranch(pid, opts),
    () => `${opts.branch} @ ${opts.ref}`,
    batch,
  );
}
...
export function deleteBranches(
  ...
  return runSimple(
    ...
    nameOf,
    'delete-branch',
    (pid) => client.deleteBranch(pid, branch),
    () => branch,
    batch,
  );
}

export function protectBranches(
  client: GitLabClient,
  ids: number[],
  nameOf: ProjectNameResolver,
  ...
  return runSimple(
    ...
    nameOf,
    'protect-branch',
    (pid) => client.protectBranch(pid, opts),
    () => opts.name,
    batch,
  );
}

export function unprotectBranches(
  client: GitLabClient,
  ids: number[],
  nameOf: ProjectNameResolver,
  branch: string,
  ...
  return runSimple(
    ...
    nameOf,
    'unprotect-branch',
    (pid) => client.unprotectBranch(pid, branch),
    () => branch,
    batch,
  );
}

export function triggerPipelines(
  client: GitLabClient,
  ids: number[],
  nameOf: ProjectNameResolver,
  ...
  return runSimple(
    ...
    'trigger-pipeline',
    async (pid) => {
      const p = await client.triggerPipeline(pid, opts);
      return p;
    },
    () => opts.ref,
    batch,
...
export function createReleases(
  ...
  return runSimple(
    ...
    nameOf,
    'release',
    (pid) => client.createRelease(pid, opts),
    () => opts.tag_name,
    batch,
  );
}

interface CloseOrMergeOptions {
  mrIid: number;
  action: MrAction;
}
export function closeOrMergeMRs(
  client: GitLabClient,
  ids: number[],
  nameOf: ProjectNameResolver,
  opts: CloseOrMergeOptions,
  batch: BatchOptions = {},
): Promise<BatchResult[]> {
  const fn =
    opts.action === 'merge'
...
interface UpdateLabelsOptions {
  mrIid: number;
  action: LabelAction;
  labels: string[];
}
export function updateMRLabelsBatch(
  client: GitLabClient,
  ids: number[],
  nameOf: ProjectNameResolver,
  opts: UpdateLabelsOptions,
  batch: BatchOptions = {},
): Promise<BatchResult[]> {
  return runSimple(
    ids,
...
export async function previewMergeRequests(
  ...
  return mapWithConcurrency(
    projectIds,
    limit,
    async (pid): Promise<MrPreviewResult> => {
      ...
      try {
        ...
        if (opts.mode === 'single') {
          ...
          if (!branchNames.has(source))
            reasons.push(`source branch '${source}' not found`);
          if (!branchNames.has(target))
            reasons.push(`target branch '${target}' not found`);
          const dup = openMrs.some(
            (m) => m.source_branch === source && m.target_branch === target,
          );
          if (dup)
            reasons.push(`open MR ${source} -> ${target} already exists`);
        } else {
          const steps = opts.chainSteps ?? [];
          detail = steps.join(' -> ');
          const missing = steps.filter((s) => !branchNames.has(s));
          if (missing.length)
            reasons.push(`missing branches: ${missing.join(', ')}`);
        }
      } catch (e) {
...
export { GitLabApiError } from './gitlab-api';
